...
class Tetrimino: # Class for each tetrimino piece
    def __init__(self, mino, color):
        pygame.sprite.Sprite.__init__(self)
        self.tetrimino = Mino.makeTetriminos(mino)
        self.setMino(mino)
        """self.color = color
        self.x = 160
        self.y = 160
    ...
    # Update the matrix of 1's and 0's
    def matrixUpdate(self, matrix):
        for part in self.getMino():
            matrix[((part[1] + self.y) // 40), ((part[0] + self.x) // 40)] = 1
...
class Mino(pygame.sprite.Sprite):
    ...
    def makeTetriminos(self, minosList, color):
        tetrimino = []
        for mino in minosList:
            thisMino = Mino(mino[0], mino[1], color)
            tetrimino.append(thisMino)
...
while True:
    ...
    while not currentMino.hit(matrix):
        ...
        if not currentMino:
            schedule.cancel_job(currentMino.moveDown)
